...
app.get('/api/admin/google/auth-url', authMiddleware, adminMiddleware, async (req, res) => {
  if (getAdminRole(req.userEmail) !== 'owner') {
    return res.status(403).json({ error: 'Only k245620@nu.edu.pk can sync Google Calendar' })
  }
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO google_calendar_connections
       (user_id, access_token, refresh_token, scope, token_type, expiry_date, connected_email, updated_at)
...
app.get('/api/admin/google/available-times', authMiddleware, adminMiddleware, async (req, res) => {
  ...
  const availability = await pool.query(
    `SELECT id, start_time, end_time
     ...
     WHERE owner_user_id = $1 AND start_time >= $2 AND start_time <= $3
     ORDER BY start_time`,
    [owner.id, dayStart.toISOString(), dayEnd.toISOString()]
  )
  const slots = availability.rows.map((slot) => ({
    id: slot.id,
  ...
  try {
    ...
    const checkedSlots = slots.map((slot) => ({
      ...slot,
      available: !busyTimes.some((busySlot) => {
        const busyStart = new Date(busySlot.start || '')
        const busyEnd = new Date(busySlot.end || '')
        return new Date(slot.start) < busyEnd && new Date(slot.end) > busyStart
      }),
    }))
...
app.post('/api/admin/availability', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    if (getAdminRole(req.userEmail) !== 'owner') {
      return res.status(403).json({ error: 'Only k245620@nu.edu.pk can add availability' })
    }
    ...
    const result = await pool.query(
      `INSERT INTO availability_slots (owner_user_id, start_time, end_time)
...
app.delete('/api/admin/availability/:id', authMiddleware, adminMiddleware, async (req, res) => {
  if (getAdminRole(req.userEmail) !== 'owner') {
    return res.status(403).json({ error: 'Only k245620@nu.edu.pk can remove availability' })
  }
...
app.post('/api/admin/tasks', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    if (getAdminRole(req.userEmail) !== 'assigner') {
      return res.status(403).json({ error: 'Only wasi@gmail.com can assign tasks' })
    }
    ...
    const matchingSlot = await pool.query(
      `SELECT id FROM availability_slots
       WHERE owner_user_id = $1 AND start_time <= $2 AND end_time >= $3
       LIMIT 1`,
      [owner.id, start_time, end_time]
    )
    if (matchingSlot.rows.length === 0) {
      return res.status(400).json({ error: 'Choose one of the available times from k245620@nu.edu.pk' })
    }
